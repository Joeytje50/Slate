...
log('Starting process, PID: ' + (process.pid+''));
log('Starting HTTP server on port ' + (process.env.PORT||80+''));
var n=os.networkInterfaces();for(var i in n){for(var x in n[i]){if(n[i][x].family=="IPv4"){console.log("Network adapter:",i,"IP-Address:",n[i][x].address)}}}
	...
	fs.lstat(exchange.location, function(err, info){
		if(err){
			...
			reader.on('open', function(){
				...
				exchange.res.setHeader('Access-Control-Allow-Origin', '*');
...
function log(e){var t=["Jan","Feb","Mar","Apr", "May", "Jun","Jul","Aug","Sep","Oct","Nov","Dec"],n=new Date,r=n.getDate(),i=n.getMonth(),s=n.getHours();min=n.getMinutes();sec=n.getSeconds();console.log((r+" "+t[i]+" "+pad(s)+":"+pad(min)+":"+pad(sec)+" - ")+e)}
function fnf(exchange, err){
	log('File: '+exchange.location+' not found, error:', err);
